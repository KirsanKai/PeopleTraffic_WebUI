class Logic {
    ...
    toInitialCoordination() {
        ...
        const fieldDiagonal = Math.sqrt(this.data.fieldWidth * this.data.fieldWidth + this.data.fieldHeight * this.data.fieldHeight);
    ...
    toScreenAdjustment() {
        ...
        let ok = true;
        while (ok) {
            if (this.data.activeBuilds.length != this.struct.Level[this.data.level].BuildElement.length) {
                this.data.scale -= 1;
                this.updateBuildsInCamera();
            } else {
                ok = false;
